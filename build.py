...
if (subprocess.run(["javac","-encoding","utf8","-cp",(";" if os.name=="nt" else ":").join(ml),"-d","build"]+jfl).returncode!=0):
	sys.exit(1)
with zipfile.ZipFile("build/opencv_videocapture.jar","w") as zf:
	print("Writing: META-INF/MANIFEST.MF")
	zf.write("manifest.mf",arcname="META-INF/MANIFEST.MF")
	print("Writing: com/krzem/opencv_videocapture/modules/opencv_java420.dll")
	zf.write("src/com/krzem/opencv_videocapture/modules/opencv_java420.dll",arcname="com/krzem/opencv_videocapture/modules/opencv_java420.dll")
...
if ("--run" in sys.argv):
	subprocess.run(["java","-Dfile.encoding=UTF8","-jar","build/opencv_videocapture.jar"])
